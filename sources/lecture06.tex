...
Brute-force algorithm $\bigo(mnp)$. (In case of square matrices its $\bigo(n^3)$.) A line of algorithmic research improves the complexity of the algorithm: Strassen's algoritm $\bigo(n^{\log_2 7})$, ..., Le Gall $\bigo(n^{2.3728639})$. It is not of our concern, since the algorithms (beside Strassen's) are hugely impractical. 
